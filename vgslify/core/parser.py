...
def parse_conv2d_spec(spec: str) -> Conv2DConfig:
    ...
    if len(conv_filter_params) < 3:
        raise ValueError(
            f"Conv layer {spec} has too few parameters. "
            "Expected format: C(s|t|r|l|m)<x>,<y>,<d> or "
            "C(s|t|r|l|m)<x>,<y>,<s_x>,<s_y>,<d>"
        )
    if len(conv_filter_params) > 5:
        raise ValueError(
            f"Conv layer {spec} has too many parameters. "
            "Expected format: C(s|t|r|l|m)<x>,<y>,<d> or "
            "C(s|t|r|l|m)<x>,<y>,<s_x>,<s_y>,<d>"
        )
    ...
    return Conv2DConfig(
        activation=activation, kernel_size=kernel_size, strides=strides, filters=d
    )
...
def parse_pooling2d_spec(spec: str) -> Pooling2DConfig:
    ...
    if pool_type not in ["Mp", "Ap"]:
        raise ValueError(
            f"Invalid pooling type '{pool_type}' in {spec}. "
    ...
    if len(pool_stride_params) not in [2, 4]:
        raise ValueError(
            f"Pooling layer {spec} does not have the expected number of parameters. "
    ...
    if pool_x <= 0 or pool_y <= 0 or stride_x <= 0 or stride_y <= 0:
        raise ValueError(
            f"Invalid values for pooling or stride in {spec}. "
            "All values should be positive integers."
        )

    return Pooling2DConfig(
        pool_type=pool_type, pool_size=(pool_x, pool_y), strides=(stride_x, stride_y)
    )
...
def parse_dense_spec(spec: str) -> DenseConfig:
    ...
    try:
        ...
        raise ValueError(
            ...
            "Expected one of 's', 't', 'r', 'l', or 'm'."
        ) from e

    # Extract the number of neurons (units)
    units = int(spec[2:])
    if units <= 0:
        raise ValueError(f"Invalid number of neurons {units} for Dense layer {spec}.")
...
def parse_rnn_spec(spec: str) -> RNNConfig:
    ...
    if spec.startswith("B"):
        ...
        match = re.match(r"([LG])([fr])(s?)(-?\d+),?(D-?\d+)?,?(Rd-?\d+)?$", spec)
        if not match:
            raise ValueError(
                f"RNN layer {spec} is of unexpected format. "
                "Expected format: (L|G)(f|r)[s]<n>[,D<rate>,Rd<rate>]"
            )
        layer_type, direction, summarize, units, dropout, recurrent_dropout = (
            match.groups()
        )
    ...
    recurrent_dropout = (
        ...
        else int(recurrent_dropout.replace("Rd", "")) / 100
    )
...
def parse_input_spec(spec: str) -> InputConfig:
    ...
    try:
        ...
        return InputConfig(
            ...
            height=None if height == "None" else int(height) if height else None,
            channels=None
            if channels == "None"
            else int(channels)
            if channels
        ...
        raise ValueError(
            f"Invalid input string format '{spec}'. Expected valid VGSL format."
        ) from e
